...
// Import MediFlow modules
import "./AccessControl.sol";
import "./DAOProposal.sol";
import "./EHRMetadata.sol";
import "./SoulboundToken.sol";
import "./ZKQueryRegistry.sol";

contract MediFlowCore {
    // Module references
    AccessControl public accessControl;
    DAOProposal public daoProposal;
    EHRMetadata public ehrMetadata;
    SoulboundToken public soulboundToken;
    ZKQueryRegistry public zkQueryRegistry;

    address public owner;
    bool public initialized;

    event MediFlowInitialized(
        address accessControl,
        address daoProposal,
        address ehrMetadata,
        address soulboundToken,
        address zkQueryRegistry
    );
    ...
    function initialize() external onlyOwner {
        ...
        daoProposal = new DAOProposal();
        ehrMetadata = new EHRMetadata();
        soulboundToken = new SoulboundToken();
        zkQueryRegistry = new ZKQueryRegistry();

        initialized = true;

        emit MediFlowInitialized(
            address(accessControl),
            address(daoProposal),
            address(ehrMetadata),
            address(soulboundToken),
    ...
    function getDAOProposalAddress() external view returns (address) {
        return address(daoProposal);
    }

    function getEHRMetadataAddress() external view returns (address) {
        return address(ehrMetadata);
    }

    function getSoulboundTokenAddress() external view returns (address) {
        return address(soulboundToken);
    }

    function getZKQueryRegistryAddress() external view returns (address) {
        return address(zkQueryRegistry);
    }
    ...
    event QueryVerified(bytes32 indexed queryHash, address indexed researcher, uint timestamp);

    AllUserStruck[] getAllUsers;

    modifier onlyDAOMember() {
        require(daoMembers[msg.sender].exists, "Not a DAO member");
        _;
    }
   ...
   //END PATIENTS
    ...
    constructor() ERC721("MediFlowSBT", "MSBT") {
